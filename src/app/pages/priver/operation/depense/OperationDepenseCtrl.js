(function () {
  ...
  angular.module('i2sFinance.priver.operation.depense')
      .controller('OperationDepenseCtrl', OperationDepenseCtrl);

  /** @ngInject */
  function OperationDepenseCtrl($scope,
                                $rootScope,
                                fileReader,
                                $filter,
                                $uibModal,
      ...
      SousPosteBudgetaireRegleResource.getId({"id" : sousPosteBudgetaireRegle.id}).$promise
          .then((result) => {
            if (result.sousPosteBudgetaireRegle) {
              transactionDepRevModal($scope.categorie, null, result.sousPosteBudgetaireRegle, $scope.ctrl.budgetCourant,0,1, $scope.ctrl.signe, false).result.then(function(retour) {
