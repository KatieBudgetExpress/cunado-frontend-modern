...
(function () {
  ...
  angular.module('i2sFinance.priver.transaction')
      .controller('TransactionTransfertCtrl', TransactionTransfertCtrl);

  /** @ngInject */
  function TransactionTransfertCtrl($scope,
                                    $timeout,
                                    $filter,
                                    $uibModal,
                                    $location,
    ...
    if ($scope.$parent.categorie.code === "CRE") {
      $scope.typeTransfert = "EMPR";
    } else if ($scope.$parent.categorie.code === "EPA") {
      $scope.typeTransfert = "UTIL";
    }

    $scope.chargerRegleTransfertCompte = function() {
          ...
          .then((result) => {
            ...
            if (data.length > 0) {
              ...
              for (let i = 0, tot = data.length; i < tot; i++) {
                  ...
                  //
                  $scope.regleTransfertComptes[idx] = {
                    idx: idx,
    ...
    $scope.chargerRegleTransfertCompte();

    $scope.modifierTransfert = function(regleTransfertCompte) {

      transfertCompteModal($scope.boutonModif, $scope.boutonSupprimer, $scope.boutonAnnuler, $scope.titreModif, $scope.$parent.idBudget, regleTransfertCompte.idSousPosteBudgetaireRegleDe, regleTransfertCompte, $scope.$parent.signe, false, $scope.$parent.categorie.code, $scope.typeTransfert).result.then(function(retour) {
        if (retour) {
    ...
    $scope.supprimeTransfert = function(regleTransfertCompte) {
      // Supprimer la règle
      RegleResource.remove({"id": regleTransfertCompte.idRegle}).$promise
          .then(async () => {
