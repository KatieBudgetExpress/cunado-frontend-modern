(function () {
    ...
    angular.module('i2sFinance.priver.assistant')
        .controller('AssistantComptesCtrl', AssistantComptesCtrl);

    /** @ngInject */
    async function AssistantComptesCtrl($scope,
                                        $rootScope,
                                        uuid,
                                        $translate,
                                        toastr,
        ...
        $scope.remplirData = function () {
          return new Promise((resolve, reject) => {
            let promisePoste = cuServices.viPosteBudgetaire("getParCateg", 3);
                ...
                SousPosteBudgetaireResource.getAll().$promise
                    .then((result) => {
                      if (result.sousPosteBudgetaire.length > 0) {
                        ...
                        $scope.posteBudgetaires = initTableComptes(posteBudgetaire, result.sousPosteBudgetaire);
                      }
                      resolve(true);
                    }).catch((err) => {
        ...
        function initTableComptes(posteBudgetaire, sousPosteBudgetaire) {
        ...
        function ajouterData() {
            const categorie = $rootScope.arrayCategorie.find(categorie => categorie.id === 3);
        ...
        function majData(poste, sousPoste) {
            $scope.categorie = $rootScope.arrayCategorie.find(categorie => categorie.id === 3);
        ...
        function deleteData(poste, sousPoste) {
            if (sousPoste.idSousPosteBudgetaireRegle !== null) {
              ...
              SousPosteBudgetaireRegleResource.getId({"id" : sousPoste.idSousPosteBudgetaireRegle}).$promise
                  .then((result) => {
                    if (result.sousPosteBudgetaireRegle && result.sousPosteBudgetaireRegle.comptePrincipal === 1) {
                      const message = $translate.instant("GLOBALE.MESSAGE.SUPPRIMER_PRINCPAL");
        ...
        $scope.$on('valideSelectionCompte', function (event, data) {
        ...
        $scope.$on('setBudgetCompte', function (event, data) {
            $scope.idBudget = data.budget.id;
            $scope.idRegle = data.regle.id;
            $scope.idSousPosteBudgetaireRegle = data.regle.idSousPosteBudgetaireRegle;

            let promiseSelected = cuServices.viRegleSousPosteBudgetaire("getParBudgetId",$scope.idBudget, null, null, null, null, null, $scope.idSousPosteBudgetaireRegle);
            promiseSelected.then(function(value) {
              let sousPosteBudgetaireRegle = value.data;

              if (sousPosteBudgetaireRegle.length > 0) {
                  const posteTmp = $scope.posteBudgetaires.find(ele => ele.id === sousPosteBudgetaireRegle[0].idPosteBudgetaire);
                  const eleSousPoste = posteTmp.liste.find(ele => ele.id === sousPosteBudgetaireRegle[0].idSousPosteBudgetaire);
                  const categorieTmp = $scope.liste.selected || [];
                  if (!categorieTmp.find(elePoste => elePoste.id === eleSousPoste.idPosteBudgetaire)) {
                      const catTmp = $scope.categoriesPosteBudgetaire.find(ele => ele.id === eleSousPoste.idPosteBudgetaire);
                      catTmp.liste = [];
