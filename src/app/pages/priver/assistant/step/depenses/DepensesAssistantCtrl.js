(function () {
    ...
    angular.module('i2sFinance.priver.assistant')
        .controller('AssistantDepensesCtrl', AssistantDepensesCtrl);

    /** @ngInject */
    async function AssistantDepensesCtrl($scope,
                                         $rootScope,
                                         uuid,
                                         $translate,
                                         toastr,
        ...
        $scope.remplirData = function () {
          return new Promise((resolve, reject) => {
            let promisePoste = cuServices.viPosteBudgetaire("getParCateg", 2);
        ...
        function ajouterData() {
          const categorie = $rootScope.arrayCategorie.find(categorie => categorie.id === 2);
          transactionDepRevModal(categorie, null, null, $scope.idBudget, 1, 1, $scope.ctrl.signe, true).result.then(function (objetRetour) {
        ...
        function majData(poste, sousPoste) {
          $scope.categorie = $rootScope.arrayCategorie.find(categorie => categorie.id === 2);
            ...
            SousPosteBudgetaireRegleResource.getId({"id" : sousPoste.idSousPosteBudgetaireRegle}).$promise
                .then((result) => {
                  if (result.sousPosteBudgetaireRegle) {
                    transactionDepRevModal($scope.categorie, null, result.sousPosteBudgetaireRegle, $scope.idBudget, 0, 1, $scope.ctrl.signe, true).result.then(function (objetRetour) {
                      if (objetRetour) {
              ...
              transactionDepRevModal($scope.categorie, sousPoste, null, $scope.idBudget, 1, 1, $scope.ctrl.signe, true).result.then(function (objetRetour) {
                  if (objetRetour) {
                      if (objetRetour.regle) {
        ...
        $scope.$on('valideSelectionDepense', function (event, data) {
        ...
        $scope.$on('setBudgetDepense', function (event, data) {
