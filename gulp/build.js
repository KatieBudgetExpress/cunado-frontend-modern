...
//
//    .pipe(uglifyes())
//    .pipe(uglifyes()).on('error', conf.errorHandler('Uglify'))
...
gulp.task('languages', function () {
  ...
  return gulp.src(path.join(conf.paths.src, '/languages/{' + lang + '}.json'))
      .pipe(jsonMinify())
      .pipe(ngLang2Js({
          declareModule: true,
          moduleName   : 'app.languages',
          prefix       : 'languages/'
      }))
      .pipe(concat('lang.js'))
      .pipe(gulp.dest(path.join(conf.paths.dist, '/languages/')));
});

gulp.task('langdev', function () {
  var lang = 'en,fr';
  ...
  return gulp.src(path.join(conf.paths.src, '/languages/{' + lang + '}.json'))
      ...
      .pipe(concat('lang.js'))
      .pipe(gulp.dest(path.join(conf.paths.src, '/languages/')));
});
...
gulp.task('other', ['copyVendorImages'], function () {
  ...
  return gulp.src([
    ...
    path.join('!' + conf.paths.src, '/**/*.{html,scss,md}'),
...
gulp.task('build', ['config:build', 'html', 'fonts', 'languages', 'other']);

gulp.task('buildqa', ['config:buildqa', 'html', 'fonts', 'languages', 'other']);
